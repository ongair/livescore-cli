  ...
  if (status != otherStatus) {
    if (status == 'NS') {
      ...
      if (score != newer.score) {
        ...
        else if (newer.commentary && (!this.commentary || newer.commentary.length > this.commentary.length)) {
          // find the different comms

          diff = _.difference(newer.commentary, this.commentary);
          return new MatchUpdate("COM", newer, null, diff[0]['Txt']);
        }
